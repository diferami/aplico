...
$(document).ready(function() {
    ...
    $('#btn-entregado').click(function(e){
        e.preventDefault();
        service_delivered();//este no va
        //4 $('#wraper-voucher').hide();
        //5 $('#select-pay').val("E").change();
        //6 $('#code-cust').val('');
        //7 $('#service-price').val(0);
        //8 $("#show-pay-modal").trigger('click'); 
    });
...
function login(id, key){
    ...
    $.ajax({
        ...
        if(response.state=='ok'){
            ...
            // 1 getSelectCust();
            // 2 getMessage();
            // 3 messageDemonId = setInterval(getMessage, message_interval);
